...
const FLOWS = [
  ...
  },
  {
    ...
    steps: [
      ...
      { edge: 'e.fn.snapshot-week-end->mod.snapshot-week-end/close', detail: 'Pure coverage gate (hermetic tests in close.test.ts). Per-participant completeness replaced the old existence-only alreadyProcessed skip, so a partial week-end write is healed on retry instead of read as done. Deliberately NOT symmetrical with week-start/plan.ts: week-end has two write paths (close an existing Monday row; insert a mid-week buy), so "missing" has two kinds.', anchors: [['supabase/functions/snapshot-week-end/index.ts', 'classifyCloseCoverage'], ['supabase/functions/snapshot-week-end/close.ts', 'export function classifyCloseCoverage']] },
      { edge: 'e.fn.snapshot-week-end->tbl.week_snapshots#update', detail: 'Writes week_end_price onto existing Monday rows.', anchors: [['supabase/functions/snapshot-week-end/index.ts', '.update({ week_end_price']] },
      { edge: 'e.fn.snapshot-week-end->tbl.week_snapshots#upsert', detail: 'Mid-week purchases: positions with no week-start row get one here, carrying a real weighted-average entry price and entered_mid_week = true, upserted on the (league, user, week, symbol) key. This insert had NEVER succeeded before 2026-07-27 — it wrote week_start_price: null against a NOT NULL column, so every mid-week position was silently dropped from scoring. The flag is what stops the row being double-counted as a Monday holding.', anchors: [['supabase/functions/snapshot-week-end/close.ts', 'entered_mid_week: true'], ['supabase/functions/snapshot-week-end/close.ts', 'export function midWeekEntryPrice'], ['supabase/functions/snapshot-week-end/index.ts', "onConflict: 'league_id,user_id,week_number,symbol'"]] },
  ...
  {
    id: 'flow.place-trade',
    title: 'Buy or sell a stock',
    trigger: { kind: 'user', detail: 'User confirms a buy or sell in TradeModal (post-draft add/drop)' },
    steps: [
      { edge: 'e.ui.TradeModal->fn.quote', detail: 'Live quote for display before confirming. Advisory only — the fill price is re-derived server-side.', anchors: [['apps/mobile/components/TradeModal.tsx', "invoke('quote'"]] },
      { edge: 'e.ui.TradeModal->fn.record-trade', detail: 'The ONLY trade write path since DR-001 Phase 3: the client-INSERT policy on trades was dropped (20260811000002). verify_jwt=true + in-code getUser().', anchors: [['apps/mobile/components/TradeModal.tsx', "invoke('record-trade'"]] },
      { edge: 'e.fn.record-trade->pg.check_and_bump_rate_limit', detail: 'Per-user and per-IP buckets.', anchors: [['supabase/functions/record-trade/index.ts', "p_bucket: 'record-trade'"]] },
      { edge: 'e.fn.record-trade->tbl.leagues', detail: 'League rules: stake mode, budget, notional, allow_undraftable.', anchors: [['supabase/functions/record-trade/index.ts', 'allowUndraftable: league.allow_undraftable']] },
      { edge: 'e.fn.record-trade->tbl.symbols', detail: 'is_draftable gate for buys (bypassed only when the commissioner set allow_undraftable).', anchors: [['supabase/functions/record-trade/index.ts', ".from('symbols').select('is_draftable')"]] },
      { edge: 'e.fn.record-trade->tbl.trades#insert', detail: 'Validated by the shared pure module (_shared/draft-validation.ts), filled at the server-fetched price. KNOWN RACE: two concurrent buys can both pass validation — needs an atomic SECURITY DEFINER RPC.', anchors: [['supabase/functions/record-trade/index.ts', 'validateTradeAdd({']] },
    ],
  },
  {
    id: 'flow.draft-stock',
    ...
    steps: [
      { edge: 'e.ui.draft->fn.quote', detail: 'Price display for candidate picks. Advisory only.', anchors: [['apps/mobile/app/(tabs)/draft.tsx', "invoke('quote'"]] },
      { edge: 'e.ui.draft->fn.validate-and-record-pick', detail: 'The ONLY pick write path since DR-001 Phase 3: the client-INSERT policies on drafts were dropped (20260811000003).', anchors: [['apps/mobile/app/(tabs)/draft.tsx', "invoke('validate-and-record-pick'"]] },
      { edge: 'e.fn.validate-and-record-pick->tbl.leagues', detail: 'League rules: draft status, rounds, stake mode, budget, allow_undraftable.', anchors: [['supabase/functions/validate-and-record-pick/index.ts', 'allowUndraftable: league.allow_undraftable']] },
      { edge: 'e.fn.validate-and-record-pick->tbl.drafts#insert', detail: 'Legality (turn, uniqueness, draftable universe, slot category/price bracket, stake budget) decided by the shared pure module; a 23505 on pick_number is a lost race and the client refetches.', anchors: [['supabase/functions/validate-and-record-pick/index.ts', "reason: 'pick_conflict'"]] },
      { edge: 'e.fn.validate-and-record-pick->tbl.leagues#update', detail: 'On the final pick, markDraftComplete sets draft_status=completed — and NOTHING ELSE. No matchups, no initial standings, no league start/end dates: those are generated only by the paused web client, so a league drafted on mobile never enters a season. Top launch blocker; see docs/STATUS.md §4.', anchors: [['supabase/functions/validate-and-record-pick/index.ts', "update({ draft_status: 'completed' })"]] },
    ],
  },
  {
