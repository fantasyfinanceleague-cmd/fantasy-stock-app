...
- ✅ **`[I4]` narrowed** *(`fix/narrow-league-members-self-insert`, migration `20261001000000`, unmerged/unapplied)* — found 2026-09-25 that the un-narrowed `league_members_insert_self` (`user_id = auth.uid()::text` only) let any authenticated user self-insert into ANY league by UUID, bypassing `join_league_by_code`'s invite code, capacity, and draft-status gates. `WITH CHECK` now also requires `role = 'commissioner'`, caller `= leagues.commissioner_id`, and `leagues.draft_status = 'not_started'` — exactly the one legitimate remaining caller (create-league's self-insert; join and bots have their own paths, unaffected). See the migration header and `docs/security/league-members-insert-effect-test.sql` for the full insert-path inventory, the INSERT..RETURNING visibility analysis, and effect-test cases. `[I4]` still **stays** pending a server-side `create-league`, and `[I6]` (bot insert) is untouched — see below.
...
`[I1]`→create-league · `[I2a]`→update-league · `[I2b]`→draft-control · `[I3]`→delete-league · `[I4]`→create-league (**narrowed 2026-10-01**, `20261001000000`; still held pending create-league) · `[I5]`→leave-league · `[I6]`→draft-control · ~~`[I7]`→join-league~~ ✅ **RETIRED 2026-07-17** · `[I8]`/`[I9]`→schedule-gen. `[P1]` (commissioner invite INSERT) is **permanent**. **`league_members` still holds `[I4]`/`[I5]`/`[I6]`** — retires with the create-league / leave-league / draft-control wave. `[I6]` (bot insert, any member, any time) is the residual mid-draft-reshuffle vector `[I4]`'s narrowing does NOT close — it retires with `supabase/migrations/deferred/20260929000000_drop_I6_I2b.sql` once draft-control ships server-side bot seeding.
...
- **`quote` / `historical-bars` non-2xx in Expo Go** — ✅ **RESOLVED (2026-07-17): it was Alpaca (hypothesis B), not auth.** Hypothesis A was ruled out because `preview-league`/`join-league` — which call `getUser()` on the *same single mobile client* — worked in the same session, proving the session JWT attaches correctly to `functions.invoke`. Root cause: the shared **`ALPACA_API_KEY` secret held a dead pre-rotation value**, silently breaking 6 functions including the 3 cron jobs. Re-pointed; a freshness guard (`1c0d438`, deployed) now prevents retroactive fabricated scoring. **No data corruption** (gaps, not bad rows; all pre-launch test data). Full writeup: **`docs/fixes/2026-07-ALPACA_KEY_ROTATION_SCORING.md`**. *Residual:* `quote`/`place-order` use **per-user** creds (separate store) — scoped to one test account (`PK30HH40ZK`, `total_linked = 1`), trivial re-link, deferred and likely obviated by the planned self-simulated paper-trading architecture. Also open: `task_ac589751` (per-week batching), `task_d541b74b` (cumulative-vs-delta semantics).
