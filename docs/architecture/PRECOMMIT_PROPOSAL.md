...
> **Status (2026-09-24): still a proposal, but its premise changed.** A pre-commit hook
> now exists: `.githooks/pre-commit` runs `gitleaks protect --staged`, wired via
> `git config core.hooksPath .githooks` (see `CLAUDE.md` → Secret scanning). The
> "there is no hook" finding below is therefore historical. The architecture-map
> `--check` step proposed here has **not** been added to that hook; if adopted, it would
> be a second step in the same file. Evidence it is needed: by 2026-09-24 the map had
> silently drifted (a missing migration, moved call sites, and three flows still
> describing deleted functions) until it was regenerated by hand.
