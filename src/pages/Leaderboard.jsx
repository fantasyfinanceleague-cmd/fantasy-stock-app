...
export default function Leaderboard() {
  ...
  const [prices, setPrices] = useState({});        // { SYM: last }
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // warm names
        const uniq = [...new Set((rows || []).map(p => p.symbol?.toUpperCase()))].filter(Boolean);
        for (const s of uniq) {
        ...
        // live quotes (limit concurrency a bit)
        const results = await fetchQuotesInBatch(uniq);
        setPrices(results);
  ...
  // ----- Derived: per-user portfolio stats
  ...
  const openPeerModal = (userId) => {
    ...
    const rows = (byUser.get(userId) || []).map(p => {
      const sym = p.symbol?.toUpperCase();
