...
**ESLint (mobile):**
- `apps/mobile` has a minimal flat config (`eslint.config.mjs`: typescript-eslint + eslint-plugin-react-hooks) and a `lint` script (`npm run lint`). The one enforced ERROR rule is `@typescript-eslint/no-use-before-define` (`variables: true`) — the same TDZ guard the web app runs; `react-hooks/exhaustive-deps` is a warning (advisory), and its lone `eslint-disable-line` in `components/SlotBuilder.tsx` is now a real, honored directive.
- **The RN `const styles = StyleSheet.create({...})` (and `cardShadow`) at the BOTTOM of a file trips the rule** — `styles.x` is referenced in the render above its declaration. It is SAFE (the render runs after module init, so no TDZ), just lexically flagged (~1600×). We do NOT reorder `styles` to the top (huge churn, fights the RN convention); instead each such file starts with `/* eslint-disable @typescript-eslint/no-use-before-define -- RN styles-at-bottom idiom … */`.
- **Convention for NEW mobile files:** keep styles at the bottom and add that same file-top disable comment. **Trade-off to know:** that comment turns the rule OFF for the WHOLE file, so a genuine use-before-define in that file's LOGIC won't be caught there (the rule stays fully active in `lib/`, hooks, contexts, and any file without the idiom). The real TDZ bugs that existed (e.g. `league.tsx` `sortedStandings`) were FIXED by reordering, not silenced — mobile `tsc` dropped 11→9. If you want the rule live in a component file's logic, put that file's `styles` at the top instead of adding the file-disable.
