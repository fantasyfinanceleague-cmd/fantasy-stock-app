...
**Guards here are keyed on ALL-OR-NOTHING state and blind to PARTIAL state. Check the partial case first.** This is the recurring design flaw in the scoring/snapshot pipeline — four instances found, **three now fixed, one still live**:
...
3. **STILL LIVE — `snapshot-week-end`'s `alreadyProcessed`.** ANY row carrying a `week_end_price` skips the whole league. A partial week-end write cannot be healed, and its retry no-ops rather than repairing. **This is the one remaining instance** and the reason `snapshot-week-end` is still the highest-value cleanup target: it needs the coverage-gate treatment `snapshot-week-start` got in `2caf9f8`.
